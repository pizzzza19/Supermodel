...
static constexpr float RUMBLE_BOOST = 1.0f;
...
void CWheelBoard::SendConstantForce(INT8 val)
{
  ...
  if (s_gamepad && !s_haptic)
  {
    ...
    GamepadRumble(norm, norm * 0.8f);  // large=force, small=texture
  }
  else
  {
...
void CWheelBoard::SendSelfCenter(UINT8 val)
{
  ...
  if (s_gamepad && !s_haptic)
  {
    ...
    GamepadRumble(norm * 0.1f, norm * 0.0f);  // self-center spring feel
  }
  else
  {
...
void CWheelBoard::SendFriction(UINT8 val)
{
  ...
  if (s_gamepad && !s_haptic)
  {
    ...
    float norm = (float)val / 255.0f;
    GamepadRumble(norm * 0.5f, norm * 1.0f);  // friction: both motors
  }
  else
  {
    Sint16 level = NormToSint16((float)val / 255.0f);
